<?php
	...
	class userController extends mainModel{
		...
		public function registrarUsuarioControlador(){
			...
			# Almacenando datos#
			$sedeid			= $this->limpiarCadena($_POST['usuario_sedeid']);
    		...
    		$img_dir="../views/imagenes/imagenes/fotos/usuario/";
			$codigo=rand(0,100);
    		...
    		if($_FILES['usuario_foto']['name']!="" && $_FILES['usuario_foto']['size']>0){
		        ...
		        # Nombre de la foto #
		        $foto=str_ireplace(" ","_",$identificacion);
		        $foto=$foto."_".$codigo;
			...
			if($registrar_usuario->rowCount()==1){
				...
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Error",
					"texto"=>"No fue posible registrar el usuario, por favor intente nuevamente",
					"icono"=>"error"
				];
			}
		...
		public function listarUsuarios(){
			...
			foreach($datos as $rows){
				...
				if ($rows['usuario_nombre']==""){
					$nombre=$rows['rol_nombre'];
				}else{
					$nombre=$rows['usuario_nombre'];
				}
				...
				$tabla.='
					<tr>
						...
						<td>		
							...
							<a href="'.APP_URL.'userList/'.$rows['usuario_id'].'/" class="btn float-right btn-success btn-xs" style="margin-right: 5px;">Actualizar</a>
		...
		public function listarCatalogoSede($sedeid){
			$option="";
